...
seg_host_prefix=$su -l gpadmi -c "psql -Atc \"select substr(hostname,1,length(hostname) -1) from gp_segment_configuration where content = 0 and preferred_role = 'p';\"")
